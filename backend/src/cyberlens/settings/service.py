...
from cyberlens.settings.schemas import AnalystSummary, DemoSettings, DemoSettingsUpdate, SettingsStatusResponse, SystemConfigItem
...
class SettingsService:
    ...
    async def ensure_defaults(self) -> None:
        analyst_count = int((await self.session.scalar(select(func.count()).select_from(Analyst))) or 0)
        if analyst_count == 0:
        ...
        existing = await self.session.scalar(select(SystemConfig).where(SystemConfig.config_key == DEMO_CONFIG_KEY))
        if existing is None:
    ...
    async def list_analysts(self) -> list[AnalystSummary]:
        analysts = (await self.session.scalars(select(Analyst).order_by(Analyst.display_name))).all()
                ...
                (
                    await self.session.scalar(
                        select(func.count()).select_from(Case).where(
                            (Case.assigned_to == analyst.username) | (Case.assigned_to == analyst.display_name)
                        )
                    )
                )
                or 0
            )
            summaries.append(AnalystSummary.model_validate(analyst).model_copy(update={"active_cases": active_cases}))
        return summaries
    ...
    async def list_configs(self) -> list[SystemConfigItem]:
        configs = (await self.session.scalars(select(SystemConfig).order_by(SystemConfig.config_key))).all()
        return [
            SystemConfigItem(key=item.config_key, value=item.config_value, description=item.description)
            for item in configs
        ]
    ...
    async def _get_or_create_demo_config(self) -> SystemConfig:
        config = await self.session.scalar(select(SystemConfig).where(SystemConfig.config_key == DEMO_CONFIG_KEY))
        if config is None:
            await self.ensure_defaults()
            config = await self.session.scalar(select(SystemConfig).where(SystemConfig.config_key == DEMO_CONFIG_KEY))
